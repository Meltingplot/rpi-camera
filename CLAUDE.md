...
**Network-loss → reboot.** `reboot_on_wifi_disconnect.sh` is installed as a separate systemd service by `rpi-camera install`. On startup it blocks in `wait_for_initial_association` until `iw dev wlan0 link` first reports "Connected" — bounded by `INITIAL_ASSOCIATION_TIMEOUT` (default 120s) so a chip that's stuck from boot still triggers a reboot. After that, two checks run once per second: (1) `iw dev wlan0 link` — any loss of association reboots **immediately** (chip stuck, e.g. overheating); (2) gateway ping — only after `PING_FAILURES_BEFORE_REBOOT` consecutive failures (default 30s) is this a reboot trigger, as a slow safety net. The installer also pins `wlan0` to a static `10.42.0.3/24` via `nmcli` on the `preconfigured` connection. The whole system assumes a fixed point-to-point network (gateway `10.42.0.1`, camera `10.42.0.3`) — keep that in mind when changing networking code.

**Install side effects** ([cli/install.py](meltingplot/rpi_camera/cli/install.py)). `rpi-camera install` rewrites `rpi-camera.service` to use the current user/group/home, symlinks `<venv>/bin/rpi-camera` into `/usr/local/bin`, installs the wifi-watchdog service, reconfigures NetworkManager, then `daemon-reload`/`enable`/`start`s the service. Many `sudo` calls — must be run on the target Pi.
