...
**Network-loss → reboot.** `reboot_on_wifi_disconnect.sh` is installed as a separate systemd service by `rpi-camera install`. It pings `10.42.0.1` once per second and runs `reboot` after 3 consecutive failures. The installer also pins `wlan0` to a static `10.42.0.3/24` via `nmcli` on the `preconfigured` connection. The whole system assumes a fixed point-to-point network (gateway `10.42.0.1`, camera `10.42.0.3`) — keep that in mind when changing networking code.
